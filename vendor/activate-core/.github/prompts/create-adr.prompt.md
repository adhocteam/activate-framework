...
1. **Determine the next ADR number** by checking existing files in `docs/adrs/` (or the project's ADR directory). Use the format `ADR-NNN`.

2. **Create the ADR file** at `docs/adrs/ADR-NNN-${input:slug:short-kebab-case-title}.md` with this structure:
...
3. **Update the ADR index** if one exists (e.g., `docs/adrs/README.md`).
